...
export async function getChaptersByBook(
	body: GraphQLBody<{ id: string; pagination?: Pagination }>,
...
export async function getChapter(
	body: GraphQLBody<{ id: string; pagination?: Pagination } & Pagination>,
	context: IGraphQLContext
) {
...
export async function getQuoteByMovie(
	body: GraphQLBody<{ movieId: string; pagination?: Pagination }>,
	context: IGraphQLContext
) {
	const {
		req: QuotesReq,
...
export async function getQuoteByCharacter(
	body: GraphQLBody<{ characterId: string; pagination?: Pagination }>,
	context: IGraphQLContext
) {
	const {
		req: QuotesReq,
...
async function addCharacterToQuotes(quote: Quote[] | Quote, context: IGraphQLContext) {
	if (!Array.isArray(quote)) {
		if (quote.character) {
			const { req, res, next } = createRESTArgumentsFromGraphqlRequest(
				context,
				{ id: quote.character._id },
				'character',
				false
			);
