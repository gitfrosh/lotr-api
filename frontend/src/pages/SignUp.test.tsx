...
it("mismatch password requirements", () => {
    const {getByLabelText, getByText} = render(
        <MemoryRouter>
            <SignUp/>
        </MemoryRouter>
    ...
    fireEvent.click(submitButton);
    expect(passwordInput.nextElementSibling?.textContent).toBe("Password must contain at least 8 characters, one uppercase letter, one lowercase letter, and one number and special character");
})
